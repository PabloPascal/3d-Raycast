...
*   `Render.cpp/hpp` – ядро рендеринга. Содержит функции для наложения текстур и отрисовки пола/потолка.
*   `Raycast ` - функция для испускания лучей, расчета высоты стен 
*   `map` – представление игрового мира
*   `AI.cpp/hpp` – реализация логики ботов, включая алгоритм A* для поиска пути и конечный автомат (FSM) для поведения (патрулирование, преследование, атака).
